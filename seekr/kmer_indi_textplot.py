...
# line_spacing: line space between sequence and number, default is 0.2
# seqfontsize: sequence character font size, default is 72
# numfontsize: sequence position number font size, default is 40
# colorblockh: the height of the highlight color block, default is 0.2
...
#                    seqfontsize=28, numfontsize=18, colorblockh=0.5, outputpath='', plotformat='pdf', plotdpi=300)
...
def kmer_indi_textplot(seqfile, words, color_vec='default', wraplen=60, char_spacing=1.0, line_spacing=0.5,
                       seqfontsize=28, numfontsize=18, colorblockh=0.5, outputpath='', plotformat='pdf', plotdpi=300):
    ...
    for seq, plotname in zip(seqs, plotnames):
        ...
        fig, ax = plt.subplots(figsize=(wraplen*char_spacing, max_total_lines*(1+1*line_spacing)))
        
        # set axis limits
        ax.set_xlim([0, wraplen*char_spacing])
        ax.set_ylim([0, max_total_lines*(1+1*line_spacing)])
        ...
        for i in range(total_lines):
            for j in range(wraplen):
                ...
                if i < len(wrapped_seq) and j < len(wrapped_seq[i]):
                    ...
                    rect1 = Rectangle((j*char_spacing - rect_width/2, max_total_lines*(1+1*line_spacing) - i*(1+1*line_spacing) - rect_height/2), rect_width, rect_height, color=color, linewidth=0)
                    ...
                    ax.text(j*char_spacing, max_total_lines*(1+1*line_spacing) - i*(1+1*line_spacing), wrapped_seq[i][j], fontsize=seqfontsize, color='#000000',ha='center', va='center', weight=weight)
                     
                if (i*wraplen + j) < len(seq):
                    ax.text(j*char_spacing, max_total_lines*(1+1*line_spacing) - i*(1+1*line_spacing) -line_spacing, str(i*wraplen+j+1), fontsize=numfontsize, ha='center', va='center')

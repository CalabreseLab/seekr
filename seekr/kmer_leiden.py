...
### input:
# inputfile: the route to your input fasta file, please ensure all input sequence has unique header, this will be used for node label
...
# outputname: the name of your output file, default is 'test', this will broadcase to all saved images (pdf) and csv files
# outputpath: the route to your output file, default is current working directory, example: '/Users/username/Desktop/'
...
#             edgecolormethod='gradient', edgethreshold=0.1, 
#             outputpath='', outputname='test', savecsv=True)
...
def kmer_leiden (inputfile, mean, std, k, algo='RBERVertexPartition', rs=1.0, 
                 edgecolormethod='gradient', edgethreshold=0.1,
                 outputname='test', outputpath='', savecsv=True):
    ...
    if edgecolormethod == 'gradient':
        ...
        ig.plot(partition, vertex_label=node_names,
                layout=layout, edge_curved=False, edge_color=edge_colors,edge_width=edge_widths,
                target=f'{outputpath}{outputname}_gradient_leiden_network.pdf')
        ...
        ig.plot(partition, vertex_label=node_names,
                layout=layout, edge_curved=False, edge_color=edge_colors,edge_width=edge_widths,
                target=f'{outputpath}{outputname}_threshold_leiden_network.pdf')
    ...
    if savecsv == True: 
        ...
        # Write nodes to CSV
        df_out.to_csv(f'{outputpath}{outputname}_nodes_leiden.csv', index=False)
        ...
        # Write edges to CSV
        df_triu.to_csv(f'{outputpath}{outputname}_edges_leiden.csv', index=False)

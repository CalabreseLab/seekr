...
# for better visualization and customization, please set savecsv to a customized name (string) and use Gephi with the saved nodes and edges files
...
# outputname: the name and path of your output file, default is None, which suppress the plotting 
# outputname example: 'test' this will broadcast to all saved images
...
# outputname can be set to another folder: '/Users/username/Desktop/test'
# savecsv: the name and path of the nodes and edges csv files. Default is None, which suppress saving these files. 
# savecsv example: 'testdata', output csv files will be added a trailing part _edges_leiden.csv or _nodes_leiden.csv for edges and nodes, respectively
...
# if savecsv is set to a string, the corresponding edges and nodes csv files (savecsv_edges_leiden.csv and savecsv_nodes_leiden.csv) will also be saved, which can be readily imported to Gephi
...
def kmer_leiden (inputfile, mean, std, k, algo='RBERVertexPartition', rs=1.0, 
    ...
    if savecsv: 
        ...
        # Write nodes to CSV
        df_out.to_csv(f'{savecsv}_nodes_leiden.csv', index=False)
        ...
        # Write edges to CSV
        df_triu.to_csv(f'{savecsv}_edges_leiden.csv', index=False)

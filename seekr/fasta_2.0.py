...
class RandomMaker(Maker):
    ...
    Updates 2023
    ------------
    ushuffle was removed from installation requirements and hence here
    as the cython wrapper is out of date and causing compatability issues during installation
    please refer to MEME suite (fasta-shuffle-letters) for creating ramdonly generated sequences while preserving the kmer content
    https://meme-suite.org/meme/doc/fasta-shuffle-letters.html
    the function uses the same ushuffle algorithm but on a C platform, which avoids all the python compatibility issues
    """
    ...
    def synthesize_random(self):
        ...
        print('fasta-shuffle-letters uses the same ushuffle algorithm for preserving kmer content while shuffle seuqnces.')        

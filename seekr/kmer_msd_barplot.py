...
# outputname: the name and path to save the output file, default is current working directory 'test', other example: '/Users/username/Desktop/test'
# a trailing part, '_kmer_msd_barplot', will be automatically added to the outputname
...
### Output:
# barplot named as outputname_kmer_msd_barplot.pformat
...
def kmer_msd_barplot(inputfile, mean, std, k, log2='Log2.post', sortstat='mean',sortmethod='descending', 
    ...
    if pformat in formatlist:
        plt.savefig(f'{outputname}_kmer_msd_barplot.{pformat}', format=pformat, dpi=pdpi, bbox_inches='tight')
    else:
        print("plotformat not supported. use default 'pdf' now. other common formats are: 'png', 'jpg', 'svg', 'eps', 'tif', 'tiff', 'ps', 'webp'")
        plt.savefig(f'{outputname}_kmer_msd_barplot.pdf', format='pdf', dpi=pdpi, bbox_inches='tight')

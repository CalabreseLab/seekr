...
the corresponding nodes and edges files will be saved as test_nodes_leiden.csv and test_edges_leiden.csv under current directory
    ...
    $ seekr_kmer_leiden ldseq.fa mean_4mer.npy std_4mer.npy 4 -a RBERVertexPartition -r 1.0 -ec gradient -et 0.1 -m 100 -o test -s
...
def console_find_dist():
    ...
    parser.add_argument("-mdl", "--models", default='common10', help="groups of candidate models for fitting, options are: 'all','common10' (default), or a list of distributions by user input for example ['norm','expon','pareto'].")
    parser.add_argument(
...
def console_kmer_leiden():
    ...
    parser.add_argument("-m", "--margin", default=100, help="the margin used in igraph plot, set it to be large so that the node label will not be cut off.")
    parser.add_argument(
        "-o", 
        "--outputname", 
        default="test", 

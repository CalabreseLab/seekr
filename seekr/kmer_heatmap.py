...
# x_tick_size: the font size to use for the x tick labels or the column labels. Default is 16
# y_tick_size: the font size to use for the y tick labels or the row labels. Default is 16
# cbar_font_size: the font size to use for the colorbar ticks. Default is 16
# outputpath: the path to save the output heatmap. Default is current working directory ''. another example is '/Users/username/Desktop/'
# outputname: the name of the output heatmap with the trailing part _kmer_heatmap. Default is 'test': the output heatmap will be saved as test_kmer_heatmap
...
### Output:
# user defined format of file (pdf as default) of a heatmap with both rows and columns dendrograms with the name outputname_kmer_heatmap.hformat
...
#             outputpath='', outputname='test',hformat='pdf', hdpi=300)
...
def kmer_heatmap(df, datamin, datamax, thresh_value=0.05,
                 color_range=['#1b7837', '#ffffff', '#c51b7d'], 
                 distmetric='correlation', linkmethod='complete',  
                 hmapw_ratio=0.3, hmaph_ratio=0.3, x_tick_size=16, y_tick_size=16, cbar_font_size=16, 
                 outputpath='', outputname='test',hformat='pdf', hdpi=300):
    ...
    #plt.savefig(f'{outputpath}{outputname}_kmer_heatmap.{hformat}', format=hformat, dpi=hdpi, bbox_inches='tight')
    ...
    if hformat in formatlist: 
        ...
        plt.savefig(f'{outputpath}{outputname}_kmer_heatmap.pdf', format='pdf', dpi=hdpi, bbox_inches='tight')

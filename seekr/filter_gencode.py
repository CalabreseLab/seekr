...
# match exact isoform number and wildcard * for digit 0-9
def match_string(pattern, s):
    # Replace '*' with '[0-9]' for a single digit wildcard match
    regex_pattern = pattern.replace('*', '[0-9]')
    # Check if the string matches the pattern
    return bool(re.match(f'^{regex_pattern}$', s))
    ...
    for kv in key_value_pairs:
        ...
        if key == 'transcript_name':
            ...
            if iso.isdigit() and len(iso) == 3:
                isoform_match = match_string(isoform,iso)

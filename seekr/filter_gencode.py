...
# outputname: the name of the output fasta file, default is 'test', a trailing part '_filtered.fa' will be added to the outputname


#### Output:
# fasta file (.fa) with the name of outputname_filtered.fa that is filtered by length and/or by transcript feature type and Ensemble_canonical tag and/or isoform number
...
def filter_gencode(fasta_path, gtf_path=None, len_threshold=0, canonical=False, isoform='0', outputname='test'):
    ...
    if canonical == True or isoform != '0':
        ...
        if canonical == True: 
            ...
            if len(tids_set - set(headers_tids)) > 50:
                print('There are more than 50 transcript_ids in gtf file that cannot be matched to the fasta headers.')
    ...
    with open(f'{outputname}_filtered.fa', 'w') as f:

...
### Input:
# inputseq: fasta file containing background sequences. By default (inputseq='default') use mouse vM25 Long non-coding RNA transcript sequences
...
# plotfit: True (default) or False. whether or not to plot the fitted distributions (red dash line) vs the actual data (blue solid line) for all distributions in models.
# if set to True, the plot will be saved as a pdf file named 'fitted_model_plots.pdf' in the current directory
...
### Example:
# fitres = find_dist(inputseq='default', k_mer=4, models='common10', subsetting=True, subset_size = 10000, fit_model=False, statsmethod='ks',progress_bar=True, plotfit=True)
...
def find_dist(inputseq='default', k_mer=4, log2='Log2.post', models='common10', subsetting=True, subset_size = 100000, fit_model=True, statsmethod='ks',progress_bar=False, plotfit=True):
    ...
    if inputseq == 'default':
        ...
        inputseq = 'gencode.vM25.lncRNA_transcripts.unique.genesequence_withfullairn.fa'
    ...
    if fit_model:
        ...
        if plotfit:
            ...
            fig, axes = plt.subplots(n_rows, n_cols, figsize=(15, n_rows*3))  # Increase figure size as necessary
            ...
            plt.tight_layout()
            # save plot as pdf
            plt.savefig('fitted_model_plots.pdf',dpi=300,format='pdf')

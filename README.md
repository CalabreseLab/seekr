...
#### kmer_leiden
Plot Leiden community network (outputname_gradient/threshold_leiden_network.pdf ) for input fasta seqeunces: calculte sequences distance matrix as seekr pearson correlation of the fasta file to itself. After filtering the edges with pearsoncutoff, use Leiden algorithms to call cluster. Seed can be set to get reporducible community assignments. The resulting undirected network edges are weighted with the pearson correlation r values: darker and thicker edges have higher weights. It works better with small number of nodes (less than 50). For better visualization and customization, please set savecsv to a string (customized name) and import the saved nodes and edges files (savecsv_edges_leiden.csv and savecsv_nodes_leiden.csv) directly to Gephi.
...
kmer_leiden.kmer_leiden(inputfile='testld.fa',mean='mean_4mers.npy',std='std_4mers.npy', 
                        ...
                        outputname='test', savecsv=None)
```

Console example (minimal and full versions):
```
$ seekr_kmer_leiden ldseq.fa mean_4mer.npy std_4mer.npy 4 -s testdata

$ seekr_kmer_leiden ldseq.fa mean_4mer.npy std_4mer.npy 4 -a RBERVertexPartition -r 1.0 -pco 0.1 -sd -ec threshold -et 0.2 -m 12 -o test -s testdata
```
...
```
docker pull lysosomelee/seekr_docker:latest
```
...
```
docker run -v /path/to/your/files:/data lysosomelee/seekr_docker:latest seekr_kmer_textplot /data/seq1.fa /data/seq2.fa 'ATTA,AAAA,ACTC' -o /data/test
```
In this command:
...
```
docker run -v e:/test:/data lysosomelee/seekr_docker:latest seekr_kmer_textplot /data/test1.fa /data/test2.fa 'ATTA,AAAA,ACTC' -o /data/test
```
```
docker run -v e:/test:/data lysosomelee/seekr_docker:latest seekr_kmer_leiden /data/testld.fa /data/bkg_mean_4mers.npy /data/bkg_std_4mers.npy 4 -s -o /data/test
```
Basically you need to add: `docker run -v /path/to/your/files:/data lysosomelee/seekr_docker:latest` before the command line code for seekr (see above for examples of all functions).
...
```
docker run -it -p 8888:8888 -v /path/on/host:/path/in/container lysosomelee/seekr_docker:latest /bin/bash
```
...
Example of code:
```
docker run -it -p 8888:8888 -v e:/test:/data lysosomelee/seekr_docker:latest /bin/bash
```
